...
pacman::p_load_gh("csgillespie/efficient")
```

# Examples on improving your code's performance
...
A dataframe is a useful object as it can contain data of different data types. For example, in our data frame it has a column of numbers and a column of characters. This is great for us because all of the data is stored in the same object/place. But this comes at the cost of processing speed.

The first two methods filter by the row first and then the column. By filtering by the row first the interm we filter the data but are still bound to a data frame object and its associated speed penalty.
...
Even when a single data type is present within the data frame the speed penalty still remains. This is because when using a dataframe R always has to check for the presence of different data types.
...
dice_roll_apply=sapply(1:100,function(i) sample(1:6,1))
...
microbenchmark(apply(ex_mat,2,function(i) mean(i)/sd(ex_mat)),
               apply(ex_mat,2,function(i) mean(i)/cached_sd))
...
dice_game=function(n_rolls=100){
  ...
  for(i in 3:n_rolls){
    ...
    if(dice_rolls[i]==dice_rolls[i-1] && dice_rolls[i]==dice_rolls[i-2]){
...
What about if we want to simulate this game many times and then estimate how often we win. Can we run this in parallel?
...
parSapply(clust, seq_len(N_games), function(i) dice_game()),
times=1
)
...
```{r df_good_naming}
avg_england_temp=data.frame('Month Number'=1:12,
...
As a shortcut you can highlight your code and press "Ctrl + Alt + A" to automatically reformat the code.
