...
We can remove the 'data_coalesce' object, as it has oer 1m rows and we're not using it.
...
We use use group_by(), summarise() and ungroup() to now aggregate the data.
...
First we need to sum the figures in the 'count_by_sla' column, we can then use the percent() function {scales} to generate some proportions.
