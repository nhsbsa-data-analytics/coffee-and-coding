...
If you have the files locally, you can use selectFile() within {rstudioapi} to select the file you want to load manually.
This can be useful if you are new to using R.

```{r}
# Alternatively load data using the rstudioapi package
data = readr::read_csv(rstudioapi::selectFile())
```

With the data loaded by one of the above two methods, we can inspect the data.
