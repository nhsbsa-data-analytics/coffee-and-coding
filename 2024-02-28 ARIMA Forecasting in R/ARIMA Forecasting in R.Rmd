...
This session will follow the process laid out in the book [Forecasting: Principals and Practice](https://otexts.com/fpp3/arima-r.html). This book goes in to a great amount of detail on modelling and forecasting and I cannot reccomend it enough as a great place to get further information on everything covered in today's session.
...
* MA: Moving average - this component of the model tells us how our oservation at the current time-step is dependent upon previous errors.
...
Be aware there are other types of transformations which we will not cover in this session. Logging your data is in fact a special case of a wider method of transformations called Box-Cox transformations and if your data was particularly skewed it may be you need to apply a different variation of a Box-Cox transformation. Also note that it can be necessary to apply a second round of differencing to your data if it still looks non-stationary after transforming and differencing once. 
...
* q is the order of the moving average component, it is the number of significant lagged errors that an observation depends on.
...
The most interesting column for us in this output is the AICc. When comparing models we want to chose the model which minimises this column. From the two models we have above the model where we turned off stepwise searching has a lower AICc value so this is the optimum model. 
...
Other tests such as Portmanteau tests can be carried out on the residuals but we will not cover them in this session today.
...
* Q is the order of the seasonal moving average component, it is the number of significant lagged seasonal errors that an observation depends on.
