...
R with web applications. - Simple annotations to define API endpointss -
...
For example, to extract data from the April 2024 dataset, specically
...
apostrophe (`'`) will be encoded as `%27`. -The greater than or equal to
sign (`>=`) will be encoded as `%3E%3D` etc.
...
practice in North East & Yorishire.
...
# Check the status of the response, 200 means susccess
...
# Perform a request to fetch the results using httr2
response <- request(url) |> 
  httr2::req_perform() #httr2 function
