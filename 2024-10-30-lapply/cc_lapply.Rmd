...
Before we do that, we will look at two diffferent types of function in R.
...
You can use a 'for-loop' to apply a function to a serues of inputs.
...
Namely, manually repeating lines of code, where could use a function to repeat the process for us.
...
Lets also say, we also wanted this aggregated done per practice separately (for a reporting reason of some kind).
...
That said, we can still use this to speed up a process that in the first instance was under 5 minutes.
...
If a function is being repeated 8k+ times, a saving of 0.01 seconds per iteration, would still equate to about a minute a half.
...
We don't need to define the number of cores again, but do neeed to define everything else.
...
The next example will be an better illustration of this. 
...
First, we need to revert our 'df' object, back from a data.table to a datarame.
...
I can access each plots by list index.
...
With can do this using our region names vector, along with the setNames() function
...
```{r}
# Lambda function two
save_charts_individually = function(x) saveRDS(plot_list[[x]], file = paste0("Output/", x, " plot"))
```
...
+ Specify a vector with a list of package.
