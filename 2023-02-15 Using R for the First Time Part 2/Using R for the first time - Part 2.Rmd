...
Printing the dataset by typing data, we notice a few issues with the data immediately, including:
  ...
  * There seems to be errors in certain columns, such as:
    * In the column 'How many employees does your company or organization have?', on line 2 there is a date instead of an amount of employees specified. 
...
Printing the dataset, we can see that the names have been changed.(Note we could have just used head(data) here but instead used kable to format the output a little more user friendly for the markdown document.)
...
Each column in a data frame has its own data type, and R automatically assigns a data type to a column when the data is loaded into the data frame. However, you can also explicitly specify the data type of a column when creating a data frame or convert the data type of a column later on.
...
Finally, each data frame has an index which can be either an integer or a label, and it is used to identify and reference the rows of the data frame. In the above data, you can see the index on the far left, starting from 1 and ending at 1433.
...
unique(data$age)
```
Here we see there are three values where the number is written in text form instead of a numerical. We will change these before we convert this column to an integer type, otherwise R will replace them with NA during the conversion. 

To change our data values we will again be using dplyr to 'pass' our data to the functions we want to apply, we use mutate first to say that we are going to be changing the data set and then we apply our functions within the mutate. The find and change the numbers written as text we will use a case_when statement, where we can specify conditions and the results we want for those conditions.
...
Here we can see that two future dates have been used: 2025-06-01 and 2023-05-01 which haven't occurred yet and must be wrong. We will deal with these in the next section.
...
Returning to the survey_date column from the previous section, we already identified that two future dates have been used: 2025-06-01 and 2023-05-01 which haven't occurred yet and must be a mistake. We can identify the row numbers where these errors occur using the which(...) function. 
...
As suspected, it looks like all of these responses should have occurred on the 1st Jan 2022. To replace these values we will again use an ifelse function which is similar to the case_when we used earlier but is a little simpler to use given we only have one replacement value to assign this time.
...
data <- data |> mutate(gender = case_when(is.na(gender) ~ NA,
                                          gender %in% male ~ "male",
...
```
On the flip side, conditions is made up mostly of NULLs but this is because the majority of the dataset have expressed that they don't have mental health, as we saw above. However, this field is very messy and is not likely to be useful for insights. Therefore, we can use select to drop this column in the following way
...
my_data <- my_data |> 
  ...
  mutate(age = ifelse(age %in% c(3, 15, 99, 323), round(mean(age),0), age),
         gender = case_when(is.na(gender) ~ NA,
                            gender %in% male ~ "male",
                            gender %in% female ~ "female",
                            TRUE ~ "other"),
